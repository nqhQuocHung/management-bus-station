...
public interface RouteRepository extends JpaRepository<Route, Long> {
    ...
    @Query("SELECT r FROM Route r WHERE (:name IS NULL OR r.name LIKE %:name%) AND (:isActive IS NULL OR r.isActive = :isActive)")
    List<Route> list(@Param("name") String name, @Param("isActive") Boolean isActive);

    @Query("SELECT COUNT(r) FROM Route r WHERE (:name IS NULL OR r.name LIKE %:name%) AND (:isActive IS NULL OR r.isActive = :isActive)")
